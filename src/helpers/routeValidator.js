...
exports.validateServerCapacityRoute = (req, res, next) => {
  ...
  if(!hasRequiredFields(body)) {
    return res.status(400).send({ error: "'server' and 'virtualMachines' fields are required" })
  }

  if(!serverFieldIsValid(body.server)) {
    return res.status(400).send({ error: 'server field has invalid keys or values' })
  }

  if(!virtualMachinesAreValid(body.virtualMachines)){
    return res.status(400).send({ error: 'virtualMachines array contains wrongly formatted inputs'})
  }
